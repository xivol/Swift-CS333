...
class JSONDataLoader<T>: DataLoader {
    ...
    func errorFor(response: URLResponse?, with data: Data?, error: Error?) -> NSError? {
        ...
        guard response != nil else {
            return NSError(domain: "Empty Response", code: 0, userInfo: [:])
        }
        guard data != nil else {
            return NSError(domain: "Empty Data", code: 0, userInfo: ["response": response!])
        }
        guard
            let httpResponse = response as? HTTPURLResponse,
            httpResponse.statusCode == 200
        else {
            return NSError(domain: "Unexpected Response", code: 0, userInfo: ["response": response!])
        }
